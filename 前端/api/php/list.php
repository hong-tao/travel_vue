<?php
    ...
    if($type == ''){
        ...
        if($title != ''){
            $sql = "select * from list where name or destination or origin or theme like '%$title%' and state='up'";
        }else {
            $sql = "select * from list where state='up'";
        }
    ...
    // 批量删除
        ...
        foreach ($idxs as $idx) {
            $numIdx = (int)$idx;
            $sql = "delete from list where productID=$numIdx";
            $result = $conn->query($sql);
    ...
    // 单项下架
        ...
        $sql = "update list set state='down' where productID=$id";
        $row = $conn->query($sql);
    }
    // 批量下架
        ...
        foreach ($idxs as $idx) {
            $numIdx = (int)$idx;
            $sql = "update list set state='down' where productID=$numIdx";
            $result = $conn->query($sql);
            Array_push($row, $result);
        }
    }
    // 单项上架
        ...
        $sql = "update list set state='up' where productID=$id";
        $row = $conn->query($sql);
    }
    // 批量上架
        ...
        foreach ($idxs as $idx) {
            $numIdx = (int)$idx;
            $sql = "update list set state='up' where productID=$numIdx";
            $result = $conn->query($sql);
            Array_push($row, $result);
        }
    }
    // 添加
        ...
        $sql = "insert into list $resKey values $resVal";
        $row = $conn->query($sql);
    }
    // 编辑
    else if($type == 'edit'){
        ...
        $sql = "update list set $res where productID=$id";
        $row = $conn->query($sql);
    }
